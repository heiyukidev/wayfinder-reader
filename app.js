...
const EMPTY_PREVIEW_HTML = '<p class="preview-placeholder">Load a Project.</p>'
...
function formatSkipPrompt() {
  ...
  return join(
    concat(
      [SKIP_PROMPT_PREAMBLE, `Project: ${currentProjectName}`],
      mapBlocks,
    ),
    '\n\n',
  )
...
async function selectFile(relPath) {
  ...
  try {
    if (!currentRootHandle) throw new Error('No Project loaded')
    const data = await readPreviewFile(currentRootHandle, relPath, languagePaths())
...
recentsSelect.addEventListener('change', async () => {
  const index = recentsSelect.value
  recentsSelect.value = ''
  if (index === '') return
  const recents = await listRecents()
  const row = get(recents, Number(index))

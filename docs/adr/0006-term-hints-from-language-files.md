...
Parse `## Language` and `## Glossary` (stop at the next `##`). A Term is a `**Name**:` block or a `- **Name** —` list item, not free bold. Match whole words, case-insensitive, skip `code` / `pre` / URLs; `_Avoid_` aliases hint “Prefer **Term**.” Several CONTEXT files merge; a collision shows both definitions, labeled by context. Missing language is a silent no-op. Dotted underline in existing ink; hover only; no color change.
